...
public class CNF {
	...
	private Formula removeEquivilance(Formula f) {
		if (f instanceof Equivalent) {
			...
			f = new And(new Implies(Cloner.clone(tmp.left),
					Cloner.clone(tmp.right)), new Implies(
					Cloner.clone(tmp.right), Cloner.clone(tmp.left)));
	...
	private Formula standardizeApart(Formula f, HashMap<String, String> mapping) {
		if (f instanceof Not) {
			...
			((BinaryOperator) f).left = standardizeApart(
					((BinaryOperator) f).left, mapping);
			((BinaryOperator) f).right = standardizeApart(
					((BinaryOperator) f).right, mapping);
		} else if (f instanceof Quantifier) {
			String newVar = NEW_VAR_PREFIX + (newVarIdx++);
			String oldVar = ((Quantifier) f).var.name;

			mapping.put(oldVar, newVar);
			((Quantifier) f).formula = standardizeApart(
					((Quantifier) f).formula, mapping);
	...
	private Formula skolemize(Formula f, ArrayList<Variable> boundVars,
			HashMap<String, Function> sub) {
		if (f instanceof Not) {
			((Not) f).formula = skolemize(((Not) f).formula, boundVars, sub);
		} else if (f instanceof BinaryOperator) {
			((BinaryOperator) f).left = skolemize(((BinaryOperator) f).left,
					boundVars, sub);
			((BinaryOperator) f).right = skolemize(((BinaryOperator) f).right,
					boundVars, sub);
		} else if (f instanceof ForAll) {
			boundVars.add(((ForAll) f).var);
			((ForAll) f).formula = skolemize(((ForAll) f).formula, boundVars,
					sub);
			boundVars.remove(boundVars.size() - 1);
		} else if (f instanceof ThereExist) {
			String newFunName = NEW_FUN_PREFIX + (newFunIdx++);
			Function newFun = new Function(newFunName, new ArrayList<Variable>(
					boundVars).toArray(new Term[boundVars.size()]));
	...
	private Formula distributeAndsOrs(Formula f) {
		if (f instanceof Not) {
			...
			if (((Or) f).right instanceof And) {
				...
				f = new And(new Or(Cloner.clone(other), and.left), new Or(
						Cloner.clone(other), and.right));
	...
	private Clause getClause(Formula f) {
		...
		if (f instanceof Not) {
			...
			l.negated = true;
			l.predicate = (Predicate) ((Not) f).formula;
			ret.add(l);
		} else if (f instanceof Predicate) {
			Literal l = new Literal();
			l.negated = false;
			l.predicate = (Predicate) f;
			ret.add(l);
		} else if (f instanceof Or) {
	...
	private ArrayList<Clause> clausify(Formula f) {
		...
		if (f instanceof Not || f instanceof Predicate || f instanceof Or) {
			ret.add(getClause(f));
		} else if (f instanceof And) {
	...
	public ArrayList<Clause> convertToCNF(Formula f) {
		...
		// Skolemize
		f = skolemize(f, new ArrayList<Variable>(),
	...
	public static void main(String[] args) {
		...
		Formula z = new And(new ForAll(new Variable("x"), p3), new ForAll(
				new Variable("x"), p1));

		Formula zz = new And(new ForAll(new Variable("x"), new ThereExist(
				new Variable("y"), new Predicate("Q", new Variable("y")))),
				new ThereExist(new Variable("z"), new Predicate("P",
						new Variable("z"))));

		Formula xz = new Or(new And(new Predicate("P", new Variable("x")),
				new Predicate("Q", new Variable("x"))), new And(new Predicate(
				"S", new Variable("x")), new Predicate("R", new Variable("x"))));

		Formula test = new Or(new And(new Predicate("a"), new Predicate("b")),
				new And(new Not(new Predicate("c")), new Or(new Predicate("d"),
						new Predicate("e"))));

		Formula Example1 = new ThereExist(new Variable("x"), new And(
				new Predicate("P", new Variable("x")), new ForAll(new Variable(
						"x"), new Implies(new Predicate("Q", new Variable("x")),
						new Not(new Predicate("P", new Variable("x")))))));

		Formula Example2 = new ForAll(new Variable("x"), new Equivalent(
				new Predicate("P", new Variable("x")), new And(new Predicate("Q",
						new Variable("x")), new ThereExist(new Variable("y"),
						new And(new Predicate("Q", new Variable("y")),
								new Predicate("R", new Variable("y"),
										new Variable("x")))))
